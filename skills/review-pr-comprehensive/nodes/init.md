...
1. **Resolve the PR.** If `$ARGUMENTS` contains a number or URL, use it. If a URL, extract the trailing PR number. If empty, call `gh pr view --json number -q .number` against the current branch.
2. **Verify the PR exists.** `gh pr view <N>` should succeed. If not, error: "PR #N not found, or you don't have access."
3. **Resolve report paths.** Derive `<org>/<repo>` from the source repo (prefer `upstream`, fall back to `origin`). Build:
