...
1. **Check the current branch** by running `git branch --show-current`:
   - If the branch name does not look like a mainline branch (`main`, `master`, `release`, or a `release/*` pattern), warn the user and ask them to confirm before continuing — tagging a feature branch is usually a mistake
...
2. **Fetch all tags and find the latest one** by running:
   ...
   - The latest tag is the first result; strip a leading `v` if present and parse as `MAJOR.MINOR.PATCH`
   - If the latest tag cannot be parsed as semver (e.g. `release-1.0`, `deploy-20240101`), warn the user, display the tag, and ask them to confirm the base version to use before continuing
...
4. **Determine the version bump** by scanning the commit messages using these rules in order of precedence (highest wins):
...
6. **Ask the user to confirm** before creating the tag using `AskUserQuestion`:
...
7. **Create the annotated tag** with a multi-line message summarizing the changes:
   ...
   - Use a heredoc to avoid quoting issues with special characters in bullet text:
     ```bash
...
EOF
     ...
     ```
   - Always use the `v` prefix on the tag name (e.g. `v1.2.3`)

8. **Confirm success** by running `git tag --sort=-version:refname | head -5` and displaying the output so the user can see the new tag at the top of the list.
...
- If `$ARGUMENTS` looks like a version string (e.g. `1.4.0` or `v1.4.0`): strip any leading `v`, use that version directly, and always add a `v` prefix when creating the tag name (still show the commit summary; skip steps 4 and 6)
- If `$ARGUMENTS` is a bump type (`major`, `minor`, or `patch`), override the auto-detected bump with that value
